...
const TabelaMecanicas: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMecanicas = async () => {
      try {
        const response = await fetch('http://localhost:8080/mecanica-agil/api/mecanica/all');
        if (!response.ok) {
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`http://localhost:8080/mecanica-agil/api/mecanica/delete/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const handleEditSave = async () => {
    if (currentMecanica) {
      try {
        const response = await fetch(`http://localhost:8080/mecanica-agil/api/mecanica/update/${currentMecanica.id}`, {
          method: 'PUT',
          headers: {
